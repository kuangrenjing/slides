...
在Python查找的时候，如果在sys.modules没有查找到，就会依次调用sys.meta\_path中的finder寯如下例，我们实现了一个会将每次加载包的äpath, target=None):
...
```python
 ...
 Importing cStringIO None N分为二类，一类是上一章节已经描述的meta hook¦果它不能处理该路径，就必须抛出ImportErrorï

这里Flask就是使用了import 的hook，当引入fla个叫ExtensionImporter的类，这个类实现了find\_moduorts from this submodule to other locations.
    This makes it possible to transition from the old flaskext.name to thele):
        self.module_choices = module_choices
        fix = wrapper_module + '.'
        self.prefix_cutoff = wra
    ...
    invocation now will have created a fullname entry in
           ...
           # is the one from __import__ above which we don't cmodules[fullname] = sys.modules[realname]
            if '.', modname, module)
            return module
